...
const Register = () => {
  ...
  const [userRegistration, setUserRegistration] = useState({
   ...
   useEffect(()=>{
    ...
    // axios.get("http://localhost:5000/api/auth/view").then((response) => {
    //     setUserRegistration(response.data);
    //   });
    const article = { email: localStorage.getItem('emailid') };
    axios.post('http://localhost:5000/api/auth/view', article)
        .then(response =>{
          setUserRegistration(response.data);
  ...
  //userRegistration.yop=userRegistration.yop.substring(0,10);
                      ...
                      <div className="col-md-12 mb-4 d-flex align-items-center">
                        <div className="form-outline datepicker w-100">
                          <input
                            ...
                            className="form-control"
                            placeholder="YYYY"
                            value={userRegistration.yop.substring(0,10)}
                            onChange={handleInput}
                            id="yop"
                            name="yop"
                          />
